#import "SodiumReactNative.h"
#import "sodium.h"
#include "macros.h"

@implementation SodiumReactNative
...
RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(
  crypto_secretbox_open_easy:(NSArray*)m c:(NSArray*)c n:(NSArray*)n k:(NSArray*)k)
{
  RN_ARG_BUFFER(k, crypto_secretbox_KEYBYTES, ERR_BAD_KEY)
  RN_ARG_BUFFER(n, crypto_secretbox_NONCEBYTES, ERR_BAD_NONCE)
  RN_ARG_BUFFER_NO_CHECK(c)

  unsigned long long mlen_check = clen - crypto_secretbox_MACBYTES;
  RN_RESULT_BUFFER(m, mlen_check, ERR_BAD_MESSAGE_LENGTH)

  RN_CHECK_FAILURE(crypto_secretbox_open_easy(m_data, c_data, clen, n_data, k_data))
...
RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(
  crypto_hash_sha512:(NSArray *)out in:(NSArray *) in)
{
  RN_RESULT_BUFFER(out, crypto_hash_sha512_BYTES, ERR_BAD_OUTPUT)
  RN_ARG_UCONST_BUFFER_NO_CHECK(in)

  RN_CHECK_FAILURE(crypto_hash_sha512(out_data, in_data, inlen))

  RN_RETURN_BUFFER(out)
}

RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(
  crypto_onetimeauth:(NSArray *)out in:(NSArray *) in k:(NSArray *)k)
{
  RN_RESULT_BUFFER(out, crypto_onetimeauth_BYTES, ERR_BAD_OUTPUT)
  RN_ARG_UCONST_BUFFER_NO_CHECK(in)
  RN_ARG_BUFFER(k, crypto_onetimeauth_KEYBYTES, ERR_BAD_KEY)

  RN_CHECK_FAILURE(crypto_onetimeauth(out_data, in_data, inlen, k_data))
...
RCT_EXPORT_BLOCKING_SYNCHRONOUS_METHOD(
  crypto_generichash:(NSArray *)out in:(NSArray *) in key:(NSArray *) key)
{
  RN_RESULT_BUFFER_MIN_MAX(out, crypto_generichash_BYTES_MIN, crypto_generichash_BYTES_MAX, ERR_BAD_OUTPUT)
  ...
  RN_CHECK_FAILURE(crypto_generichash(out_data, outlen, in_data, inlen, key_data, keylen))
